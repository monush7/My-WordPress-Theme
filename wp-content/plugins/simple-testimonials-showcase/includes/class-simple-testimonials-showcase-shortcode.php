...
class Simple_Testimonials_Showcase_Shortcode
{
    ...
    public function callback_simple_testimonials($atts, $content)
    {
        ...
        $shortcode_args_raw = shortcode_atts($shortcode_args_default, $atts);
        
        //  Sanitizing parameters for security
        $accepted_params = array_keys($shortcode_args_default);
        $shortcode_args = [];
        foreach( $shortcode_args_raw as $k => $v ){
            if( ! in_array( $k, $accepted_params ) ){   //  confirm to receive the shortcode's parameters
                continue;
            }
            $shortcode_args[$k] = sanitize_text_field( preg_replace( '/[^a-z0-9 ]/i', '', $v) );
        }
