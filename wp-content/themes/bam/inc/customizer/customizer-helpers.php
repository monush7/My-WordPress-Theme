...
/**
 * Check if the Google Fonts are disabled or not.
 */
function bam_is_google_fonts_active( $control ) {
	if ( $control->manager->get_setting( 'bam_disable_google_fonts' )->value() == false ) {
		return true;
	} else {
		return false;
	}
...
/**
 * Checks if bam is showing date
 */
function bam_is_showing_date( $control ) {
	if ( $control->manager->get_setting( 'bam_show_date' )->value() === true ) {
		return true;
	} else {
		return false;
	}
...
/**
 * Checks if bam is showing date in single posts
 */
function bam_is_showing_date_single( $control ) {
	if ( $control->manager->get_setting( 'bam_single_show_date' )->value() === true ) {
		return true;
	} else {
		return false;
	}
 ...
 * Checks if bam is showing date
 */
function bam_is_showing_date_any( $control ) {
	if ( bam_is_showing_date( $control ) || bam_is_showing_date_single( $control ) ) {
		return true;
	} else {
		return false;
	}
...
/**
 * Checks if bam is showing breadcrumbs
 */
function bam_is_showing_breadcrumb( $control ) {
	if ( $control->manager->get_setting( 'bam_breadcrumb_source' )->value() !== 'none' ) {
		return true;
	} else {
		return false;
	}
